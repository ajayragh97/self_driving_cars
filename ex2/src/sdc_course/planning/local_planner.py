...
class LocalPlanner:
    ...
    def _fit_cubic(
        ...
        #######################################################################
        """ 
        from the given boundary condition I am going to consider that first derivatives
        x'(u) = c_u * cos(theta_u)
        y'(u) = c_u * sin(theta_u)
        """
        # Given
        ...
        # derivatives
        x_0_prime = c0 * np.cos(theta_0)
        y_0_prime = c0 * np.sin(theta_0)
        x_1_prime = c1 * np.cos(theta_1)
        y_1_prime = c1 * np.sin(theta_1)
        
        """ 
        the equations are:
        x(u) = a3 * u^3 + a2 * u^2 + a1 * u^1 + a0
        y(u) = b3 * u^3 + b2 * u^2 + b1 * u^1 + b0
        
        their derivatives are:
        x'(u) = 3a3 * u^2 + 2a2 * u^1 + a1 + 0
        y'(u) = 3b3 * u^2 + 2b2 * u^1 + b1 + 0
        ...
        we can construct the system of equations, Ax = B, as:
        ...
        # u belongs to [0, 1], this means u ranges from 0 to 1.
        # u=0 for the starting position 
        # u=1 for the ending position
        # constructing the system of matrices based on the above description
        
        u0  =   0
        u1  =   1
        
        A = np.array([[u0**3,   u0**2,  u0, 1],
                      [u1**3,   u1**2,  u1, 1],
                      [3*u0**2, 2*u0,   1,  0],
                      [3*u1**2, 2*u1,   1,  0]])
        ...
        x = np.linalg.solve(A, B) # returned x array is of shape 4*2
        parameters = x.T

...
if __name__ == "__main__":
    ...
    try:
        ...
        while not window.should_close:
            ...
            # Pass to controller (compute_control is in the Parent's Class - AbstractController)
            # control = (acceleration, steering_angle)
            control = controller.compute_control(target_speed, target_trajectory)
            # So since the control is acceleration itself, and not the pressing Throttle/Break itself, I suppose in vehicle it is being converted
            # from the acceleration to the actual control which is how much should we press throttle/break
